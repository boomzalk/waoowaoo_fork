...
/**
 * Fallback 2: POST /v1/chat/completions (grok2api 格式)
 * 使用 chat completions 格式生成视频
 */
async function createVideoViaChatCompletions(
  baseUrl: string,
  apiKey: string,
  model: string,
  prompt: string,
  ...
  // Ensure baseURL ends with /v1, then add /chat/completions
  ...
  const videoConfig: Record<string, unknown> = {
    ...
    video_length: options.duration || 6,
    resolution_name: options.resolution || '480p',
    preset: 'custom',
  }
  ...
  if (!response.ok) {
    const text = await response.text().catch(() => '')
    throw new Error(`OPENAI_VIDEO_CHAT_COMPLETIONS_FAILED: ${response.status} ${text.slice(0, 200)}`)
  }

  const data = await response.json() as Record<string, unknown>

  // Try to extract video ID or URL from response
  // The response might be in chat completion format
  ...
  if (content && typeof content === 'string') {
    // Try to parse as JSON to get video ID or URL
    try {
      const parsed = JSON.parse(content)
      videoId = parsed.id || parsed.task_id || parsed.video_id || ''
      videoUrl = parsed.url || parsed.video_url || parsed.videoUrl || ''
    } catch {
      // If not JSON, check if it's a URL or ID
  ...
  // Fallback: try to get id from top level
  if (!videoId && !videoUrl) {
    videoId = typeof data.id === 'string' ? data.id.trim() : ''
  }

  // If we got a video URL directly (synchronous response), return it
  if (videoUrl) {
    // Extract a short ID from the URL for tracking
    const uuidMatch = videoUrl.match(/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/i)
    const shortId = uuidMatch ? uuidMatch[1] : `sync-${Date.now()}`

    // Return as synchronous result with the video URL
    throw {
      __isSyncVideoResult: true,
      videoUrl,
      id: shortId
    }
  }
  ...
  if (!videoId) {
    throw new Error('OPENAI_VIDEO_CHAT_COMPLETIONS_INVALID_RESPONSE: missing video id or url')
  }

  // If videoId is a URL, extract a shorter identifier
  // Example: https://domain.com/path/to/9ca4718c-5180-4dc5-b92b-c0d2877ada25/video.mp4
  // Extract: 9ca4718c-5180-4dc5-b92b-c0d2877ada25
  if (videoId.startsWith('http://') || videoId.startsWith('https://')) {
    // Try to extract UUID or hash from URL
    const uuidMatch = videoId.match(/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/i)
    if (uuidMatch) {
      return { id: uuidMatch[1] }
    }
    // If no UUID found, use last path segment without extension
    const pathParts = videoId.split('/').filter(p => p)
...
export class OpenAICompatibleVideoGenerator extends BaseVideoGenerator {
  ...
  protected async doGenerate(params: VideoGenerateParams): Promise<GenerateResult> {
    ...
    // Strategy: try OpenAI SDK first (/v1/videos), fallback to /v1/video/create, then /pg/chat/completions
    let videoId: string
    ...
    try {
      ...
      // If endpoint is not supported, try fallback methods
      if (!isEndpointUnsupportedError(sdkError)) {
        throw sdkError
      }

      // Try fallback 1: /video/create
      try {
        ...
        // Try fallback 2: /v1/chat/completions (grok2api format)
        if (!isEndpointUnsupportedError(fallbackError)) {
          throw fallbackError
        }

        try {
          const chatResult = await createVideoViaChatCompletions(
            config.baseUrl,
            config.apiKey,
            model,
            trimmedPrompt,
          ...
          videoId = chatResult.id
        } catch (chatError: unknown) {
          // Check if this is a synchronous video result
          if (chatError && typeof chatError === 'object' && '__isSyncVideoResult' in chatError) {
            const syncResult = chatError as { __isSyncVideoResult: boolean; videoUrl: string }
            // Return synchronous result with video URL
            return {
              success: true,
              async: false,
              videoUrl: syncResult.videoUrl,
            }
          }
          throw chatError
        }
